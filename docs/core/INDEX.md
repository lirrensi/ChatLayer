---
node_type: index
updated: 2026-08-01
---

# core Index

## Contents

- [Server Architecture & API Reference 🟢](server.md)

  → /overview/product.md (depends_on)
  → /apps/server/src/ (documents)
  → /core/web-ui.md (relates_to)
  → /nsdks/node.md (relates_to)
  → /nsdks/python.md (relates_to)
  → /nsdks/go.md (relates_to)
  → /nsdks/php.md (relates_to)
  ← /archive/2026-07-16-flat-structure/arch_index.md (supersedes)
  ← /archive/2026-07-16-flat-structure/arch_server.md (supersedes)
  ← /core/web-ui.md (depends_on)
  ← /nsdks/go.md (depends_on)
  ← /nsdks/node.md (depends_on)
  ← /nsdks/php.md (depends_on)
  ← /nsdks/python.md (depends_on)
- [Web UI Architecture 🟢](web-ui.md)

  → /overview/product.md (depends_on)
  → /core/server.md (depends_on)
  → /apps/web/src/ (documents)
  → /nsdks/node.md (relates_to)
  ← /archive/2026-07-16-flat-structure/arch_web-ui.md (supersedes)
  ← /core/server.md (relates_to)

## Tags

`api` `architecture` `deployment` `lifecycle` `server` `typescript` `vue` `web-ui`

---
*Auto-generated. Last rebuilt: 2026-08-01*
